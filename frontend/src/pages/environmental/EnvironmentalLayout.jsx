...
import './EnvironmentalLayout.css'
...
export default function EnvironmentalLayout() {
  ...
  const handleExport = async (format = 'pdf') => {
    setExporting(true)
    const result = await downloadReport('environmental', format)
    setExporting(false)
    if (!result.ok) {
  ...
  return (
    <div className="environmental-layout-wrapper">
      ...
      <section className="layout-content-wrapper">
        ...
        <header className="layout-topbar">
          <div className="layout-topbar-left">
            ...
            <h1 className="env-page-title-label">Environmental</h1>
          </div>
        ...
        {/* Subnavigation Tabs */}
        <nav className="layout-tabs" aria-label="Environmental subnavigation tabs">
          <Link to="/environmental/dashboard" className={getTabClass('/environmental/dashboard')}>Dashboard</Link>
          <Link to="/environmental/carbon-transactions" className={getTabClass('/environmental/carbon-transactions')}>Carbon Transactions</Link>
          <Link to="/environmental/emission-factors" className={getTabClass('/environmental/emission-factors')}>Emission Factors</Link>
          <Link to="/environmental/environmental-goals" className={getTabClass('/environmental/environmental-goals')}>Environmental Goals</Link>
          <Link to="/environmental/product-esg" className={getTabClass('/environmental/product-esg')}>Product ESG</Link>
        </nav>
