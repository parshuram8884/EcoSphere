...
export default function GamificationLayout() {
  ...
  const handleExport = async (format = 'pdf') => {
    setExporting(true)
    const result = await downloadReport('gamification', format)
    setExporting(false)
    if (!result.ok) {
  ...
  return (
    <div className="gamification-layout-wrapper">
      ...
      <section className="layout-content-wrapper">
        ...
        <header className="layout-topbar">
          <div className="layout-topbar-left">
            ...
            <h1 className="gam-page-title-label">Gamification</h1>
          </div>
        ...
        {/* Subnavigation Tabs */}
        <nav className="layout-tabs" aria-label="Gamification subnavigation tabs">
          <Link to="/gamification/challenges" className={getTabClass('/gamification/challenges')}>Challenges</Link>
          <Link to="/gamification/challenge-participation" className={getTabClass('/gamification/challenge-participation')}>Participation</Link>
          <Link to="/gamification/badges" className={getTabClass('/gamification/badges')}>Badges</Link>
          <Link to="/gamification/rewards" className={getTabClass('/gamification/rewards')}>Rewards</Link>
          <Link to="/gamification/leaderboard" className={getTabClass('/gamification/leaderboard')}>Leaderboard</Link>
        </nav>
