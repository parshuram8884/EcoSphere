...
export default function ReportsLayout() {
  ...
  const handleExport = async (format = 'pdf') => {
    setExporting(true)
    const result = await downloadReport('reports', format)
    setExporting(false)
    if (!result.ok) {
      ...
      <section className="layout-content-wrapper">
        ...
        <header className="layout-topbar">
          <div className="layout-topbar-left">
            ...
            <h1 className="rep-page-title-label">Compliance Reporting</h1>
          </div>
        ...
        {/* Subnavigation Tabs */}
        <nav className="layout-tabs" aria-label="Reports subnavigation tabs">
          <Link to="/reports/environmental-report" className={getTabClass('/reports/environmental-report')}>Environmental</Link>
          <Link to="/reports/social-report" className={getTabClass('/reports/social-report')}>Social</Link>
          <Link to="/reports/governance-report" className={getTabClass('/reports/governance-report')}>Governance</Link>
          <Link to="/reports/esg-summary" className={getTabClass('/reports/esg-summary')}>Summary</Link>
          <Link to="/reports/custom-report-builder" className={getTabClass('/reports/custom-report-builder')}>Custom Builder</Link>
        </nav>
